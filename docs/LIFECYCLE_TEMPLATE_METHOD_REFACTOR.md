...
| `DownstreamLibSessionScreen` | ` public new` ⚠️ | ` public new` ⚠️ | Both broken |
...
`DownstreamLibSessionScreen.Entered()` registers `KeyDownEvent` for the Escape menu.
`DownstreamLibSessionScreen.Left()` unregisters `KeyDownEvent` and calls `Reset()`.
...
| `StandardScreens/DownstreamLibSessionScreen.cs` | ` DownstreamLibSessionScreen` | ` public new Entered()` → ` protected override OnEntered()`, ` Left()` → ` OnLeft()`. ` public override BeforeLeave()` → ` protected override OnBeforeLeave()` |
